...
export const isFixed = (current: number, fixedAt: number): boolean => current <= fixedAt
export const isPinned = (current: number, previous: number): boolean => current <= previous
export const isReleased = (current: number, previous: number, fixedAt: number): boolean =>
...
export const useHeadroom = ({ fixedAt = 0, onPin, onFix, onRelease }: UseHeadroomInput = {}): boolean => {
